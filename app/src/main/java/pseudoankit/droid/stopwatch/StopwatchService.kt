...
class StopwatchService : Service() {
    ...
    * */
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val action = StopWatchAction.fromString(intent?.getStringExtra(STOPWATCH_ACTION))

        Log.d("Stopwatch", "onStartCommand Action: $action")

        when (action) {
            StopWatchAction.Start -> startStopwatch()
            StopWatchAction.Pause -> pauseStopwatch()
            StopWatchAction.Reset -> resetStopwatch()
            StopWatchAction.MoveToBackground -> moveToBackground()
            StopWatchAction.MoveToForeground -> moveToForeground()
            StopWatchAction.GetStatus -> sendStatus()
            null -> {}
        }
